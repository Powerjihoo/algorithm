...
class TagValueAPI(APISession):
    ...
    def get_current_values_all(self) -> None:
        path = "/api/Python/Value/Realtime"
        url = self.baseurl + path
        return self.request_get(url)

    def get_current_values(self, tagnames: List[str]) -> List[dict]:
        tagnames_str = ",".join(tagnames)
        path = f"/current/name/{tagnames_str}"
        url = self.baseurl + basepath + path
        return self.request_get(url)

    def get_changed_values_all(self, timestamp: str) -> None:
        path = f"/api/Python/Value/Realtime/timefilter/{timestamp}"
        url = self.baseurl + path
        return self.request_get(url)
    ...
    def get_history_data_archive(
        self, start: int, end: int, tagnames: str | List[str]
    ) -> dict:
