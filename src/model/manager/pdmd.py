...
from model.prediction.pdmd import PDMD


class PDMDManager(PredModelManagerBase, metaclass=SingletonInstance):
    # Observer
    def __init__(self):
        self.cnt_calc = 0
    ...
    def define_prediction_model(
        ...
        return PDMD(model_info.modelname, tagnames, custom_base_path=custom_base_path)
    ...
    def calc_preds(self) -> None:
        ...
        for model_key, model in list(self.items()):
            ...
            try:
                ...
                if not pred_values or np.isnan(pred_values).any():
                    continue
                ...
                pred_model.index_calculator_model.calc_index(
                    index_ary=pred_model.model_tag_data.last_tag_index_list,
                    status_ary=pred_model.model_tag_data.last_tag_index_status_list,
                )

                pred_model.set_calculated()
                self.cnt_calc += 1
            except AttributeError:
                logger.debug(f"Pred calc result is None ({model_key})")
            except Exception as e:
                logger.exception(e)
            finally:
                pred_model.model_tag_data.push_data()
