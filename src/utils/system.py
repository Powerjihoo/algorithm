...
def measure_memory(func):
    ...
    def wrapper(*args, **kwargs):
        p = psutil.Process()
        before_memory_rss = p.memory_info().rss
        before_memory_vms = p.memory_info().vms
        result = func(*args, **kwargs)
        after_memory_rss = p.memory_info().rss
        after_memory_vms = p.memory_info().vms
        rss = (after_memory_rss - before_memory_rss) / (1024**2)
        vms = (after_memory_vms - before_memory_vms) / (1024**2)
        print(
            f"PID: {p.pid} | Memory increased: {rss}MB, Memory increased(virtual): {vms}MB"
        )
        return result
